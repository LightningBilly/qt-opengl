...
void Widget::initializeGL() {
    ...
    vertexShader.compileSourceFile(":/shaders/shader/triangle.vert");

    fragmentShader.compileSourceFile(":/shaders/shader/triangle.frag");

    shaderProgram[0].addShader(&vertexShader);
    shaderProgram[0].addShader(&fragmentShader);
    shaderProgram[0].link();

    fragmentShader2.compileSourceFile(":/shaders/shader/triangle_2.frag");
...
void Widget::paintGL()
{
    ...
    auto location = shaderProgram[0].uniformLocation("outColor");
    shaderProgram[0].setUniformValue(location, QVector4D(0.4, qAbs(qSin((float)time.elapsed()/1000)), 0.3, 1.0));
